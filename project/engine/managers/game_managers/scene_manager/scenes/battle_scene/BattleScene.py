...
from project.engine.managers.game_managers.scene_manager.scenes.battle_scene.map.battle_map import BattleMap
...
class BattleScene():
    def __init__(self, main: MainLike):
        ...
        self.battle_map = BattleMap(main=self.main, map_size=self.main.settings.game_settings['battle_map_size'], tile_size=self.tile_size,
                                    battle_map_outpost=settings.game_settings['battle_map_outpost'])

        print('OAER)QAW)FS_AD)F)S')
        print(self.main.camera.get_battle_map_configuration())
    def setup(self):
        pass
