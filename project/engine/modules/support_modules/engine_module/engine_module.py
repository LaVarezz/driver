...
from project.engine.modules.UI_modules.widget_module.widgets.buttons.commands.command_types import commands
from project.engine.modules.UI_modules.widget_module.widgets.widget_types import WidgetTypes
...
class EngineModule(Module):
    ...
    def save_config_to_json(self):
        ...
        for scene in self.__data.keys():
            # КОСТЫЫЫЫЫЛЬ!
            if scene == self.main.modules.scene_module.current_scene:
                ...
                for button_id in self.__data[scene]['widgets']['buttons']:
                    button = self.main.modules.widget_module.get_widget(button_id)
                ...
                for label_id in self.__data[scene]['widgets']['labels']:
                    label = self.main.modules.widget_module.get_widget(label_id)
                    data[scene]['widgets']['labels'][label_id] = label.get_data_json_like()

                ''' сохранение панелей '''
                for panel_id in self.__data[scene]['widgets']['panels']:
                    panel = self.main.modules.widget_module.get_widget(panel_id)
