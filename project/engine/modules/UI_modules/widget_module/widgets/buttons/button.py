...
class Button(BasicWidget):
    def __init__(self, main: MainLike, cords, size, id, text, command, layer, panel, moveable=False, visible=True, enabled=True, anchor=None,
        ...
        self.text = text
        self.type = 'button'
        self.main.modules.text_module.create_text_object((0, 0), (self.width, self.height), self.surface, self.text, 1,
                                                          center=(1, 1))


    def __call__(self):
        if not self.main.modules.engine_module.able_to_change:
