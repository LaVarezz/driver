...
class Animation:
    def __init__(self, start_coordinates, finish_coordinates,  parent, game, static_picture, moveable_picture, images, effect_type, speed, scale, double):
        ...
        if self.moveable:
            if self.effect_type in ['Slash', 'Punch']:
...
class AvatarGoTO:
    ...
    def update(self):
        self.time += 0.2
        self.x += (self.e_x-self.x)*0.15
        self.y += (self.e_y-self.y)*0.15
    ...
    def draw(self):
        ...
        if not self.bar:
            if self.parent.cls == 'Warrior':
                image = pg.transform.scale(self.image[0], (self.game.size-15, self.game.size-15))
                image = pg.transform.flip(image, True, False)
            elif self.parent.cls == 'Necromancer':
                image = pg.transform.scale(self.image[0], (self.game.size + 40, self.game.size + 40))
                image = pg.transform.flip(image, True, False)
                ...
                image = pg.transform.scale(self.image[0], (self.game.size+5, self.game.size+15))
                image = pg.transform.flip(image, True, False)
            ...
            var = 200
            rect_bar_full = pg.Surface((size // 5 * 4 + 3, size // 4 + 3))
            ...
            rect = rect_bar_full.get_rect(center=(x+size, y+size//20*11+4))

            if self.parent.shield:
                rect_shield_bar = pg.rect.Rect(2, 2,
                                               int((size // 5 * 4) * self.parent.shield // self.parent.health[1]),
                                               size // 8)
                pg.draw.rect(rect_bar_full, 'Gray', rect_shield_bar)
